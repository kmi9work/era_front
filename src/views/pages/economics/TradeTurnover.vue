...
// Bulk update trade turnover
const bulkUpdateDialog = ref(false)
const bulkTurnoverValue = ref(0)
const bulkUpdating = ref(false)

// Bulk thresholds editing
const bulkThresholdsDialog = ref(false)
const bulkThresholds = ref([
  { level: 1, threshold: 1000, name: 'Уровень 1' },
  { level: 2, threshold: 2000, name: 'Уровень 2' },
  { level: 3, threshold: 5000, name: 'Уровень 3' },
  { level: 4, threshold: 10000, name: 'Уровень 4' },
  { level: 5, threshold: 20000, name: 'Уровень 5' }
])
const bulkThresholdsInputMode = ref('absolute')
const savingBulkThresholds = ref(false)
...
// Открыть диалог массового обновления товарооборота
const openBulkUpdateDialog = () => {
  bulkTurnoverValue.value = 0
  bulkUpdateDialog.value = true
}

// Массовое обновление товарооборота для всех стран (кроме Вятки)
const bulkUpdateTradeTurnover = async () => {
  if (bulkTurnoverValue.value < 0) {
    alert('Значение товарооборота должно быть неотрицательным')
    return
  }
  ...
  try {
    const response = await axios.patch(
      `${import.meta.env.VITE_PROXY}/countries/bulk_update_trade_turnover.json`,
      { turnover: bulkTurnoverValue.value }
    )
    ...
    if (response.data.success) {
      ...
      alert(`Товарооборот успешно обновлен для ${response.data.countries.length} стран: ${response.data.message}`)
    } else {
      alert('Ошибка при обновлении: ' + (response.data.error || 'Неизвестная ошибка'))
    }
  } catch (error) {
    console.error('Error bulk updating trade turnover:', error)
    alert('Ошибка при обновлении: ' + (error.response?.data?.error || error.message))
  } finally {
    bulkUpdating.value = false
  }
}

// Массовое начисление уровней товарооборота для всех стран (кроме Вятки)
const bulkAwardTradeLevelPoints = async () => {
  if (!confirm('Вы уверены, что хотите обновить уровни товарооборота для всех стран, кроме Вятки?')) {
    return
  }
  ...
  try {
    const response = await axios.patch(
      `${import.meta.env.VITE_PROXY}/countries/bulk_award_trade_level_points.json`
    )
    
    if (response.data.success) {
      // Обновляем локальные данные
      await fetchTradeTurnovers()
      alert(`Уровни товарооборота успешно обновлены для ${response.data.countries.length} стран: ${response.data.message}`)
    } else {
      alert('Ошибка при обновлении: ' + (response.data.error || 'Неизвестная ошибка'))
    }
  } catch (error) {
    console.error('Error bulk awarding trade level points:', error)
    alert('Ошибка при обновлении: ' + (error.response?.data?.error || error.message))
  } finally {
    bulkUpdating.value = false
  }
...
// Открыть диалог массового редактирования порогов
const openBulkThresholdsDialog = () => {
  // Копируем текущие пороги для редактирования
  bulkThresholds.value = JSON.parse(JSON.stringify([
    { level: 0, threshold: 0, name: 'Нет' },
    { level: 1, threshold: 1000, name: 'Низкий' },
    { level: 2, threshold: 5000, name: 'Средний' },
    { level: 3, threshold: 20000, name: 'Высокий' }
  ]))
  bulkThresholdsDialog.value = true
}

// Добавить новый уровень
const addBulkThreshold = () => {
  const newIndex = bulkThresholds.value.length
  const prevThreshold = bulkThresholds.value[newIndex - 1]?.threshold || 0
  bulkThresholds.value.push({
    level: newIndex,
    threshold: prevThreshold * 2 || 1000,
    name: ''
  })
  // Пересчитываем проценты для всех
  bulkThresholds.value.forEach((_, i) => {
    if (i > 0) recalculateBulkPercentage(i)
  })
}

// Удалить уровень
const removeBulkThreshold = (index) => {
  if (bulkThresholds.value.length <= 1) {
    alert('Должен остаться хотя бы один уровень')
    return
  }
  bulkThresholds.value.splice(index, 1)
  // Перенумеровываем уровни
  bulkThresholds.value.forEach((t, i) => {
    t.level = i
  })
...
// Пересчет порогов при изменении процента
const recalculateBulkPercentage = (index) => {
  if (index < 1) return
  
  const prevThreshold = bulkThresholds.value[index - 1].threshold
  const currentThreshold = bulkThresholds.value[index].threshold
  
  if (prevThreshold > 0) {
    bulkThresholds.value[index].percentage = Math.round((currentThreshold / prevThreshold) * 100)
  }
}

// Пересчет порогов при изменении процента
const recalculateBulkFromPercentage = (index) => {
  if (index < 1) return
  
  const prevThreshold = bulkThresholds.value[index - 1].threshold
  const percentage = bulkThresholds.value[index].percentage
  
  if (percentage && percentage > 0) {
    bulkThresholds.value[index].threshold = Math.round((prevThreshold * percentage) / 100)
  }
}

// Пересчет всех последующих порогов
const recalculateBulkAllFromIndex = (startIndex) => {
  for (let i = startIndex + 1; i < bulkThresholds.value.length; i++) {
    if (bulkThresholdsInputMode.value === 'percentage') {
      recalculateBulkFromPercentage(i)
    } else {
      recalculateBulkPercentage(i)
    }
  }
}

// Сохранить массовые пороги
const saveBulkThresholds = async () => {
  savingBulkThresholds.value = true
  try {
    // Подготавливаем данные для отправки (удаляем поле percentage)
    const thresholdsToSave = bulkThresholds.value.map(({ level, threshold, name }) => ({
      level,
      threshold,
      name: name || ''
    }))
    ...
    const response = await axios.patch(
      `${import.meta.env.VITE_PROXY}/countries/bulk_update_trade_thresholds.json`,
      { thresholds: thresholdsToSave }
    )
    ...
    if (response.data.success) {
      // Обновляем локальные данные
      await fetchTradeTurnovers()
      bulkThresholdsDialog.value = false
      alert(`Пороги успешно обновлены для ${response.data.countries.length} стран: ${response.data.message}`)
    } else {
    ...
    savingBulkThresholds.value = false
  }
}

// Отменить редактирование
const cancelBulkThresholds = () => {
  bulkThresholdsDialog.value = false
  bulkThresholds.value = []
}
</script>
...
<template>
      ...
      <VCardTitle class="d-flex justify-space-between align-center flex-wrap gap-2">
        ...
        <div class="d-flex gap-2 flex-wrap">
          <v-btn
            ...
            :disabled="isLoading"
          >
            <v-icon icon="ri-global-line" class="mr-2" />
            Обновить товарооборот
          </v-btn>
          <v-btn
            color="warning"
            @click="openBulkThresholdsDialog"
            :disabled="isLoading"
          >
            <v-icon icon="ri-settings-3-line" class="mr-2" />
            Обновить пороги
          </v-btn>
          <v-btn
            color="primary"
  ...
  <!-- Модальное окно массового обновления товарооборота -->
  <v-dialog
  ...
  >
    <v-card>
      <v-card-title class="d-flex justify-space-between align-center">
        <span>Массовое обновление товарооборота</span>
        <v-btn
          icon="ri-close-line"
          variant="text"
      ...
      <v-card-text>
        <v-alert type="info" variant="tonal" class="mb-4">
          <strong>Внимание:</strong> Это действие обновит товарооборот для всех стран, кроме Вятки (караваны через Вятку не будут затронуты).
        </v-alert>
        
        <v-text-field
          v-model="bulkTurnoverValue"
          label="Значение товарооборота"
          type="number"
          min="0"
          outlined
          dense
          prefix="Золото: "
          hint="Товарооборот будет установлен для всех иностранных государств"
          persistent-hint
        />
      </v-card-text>
      <v-card-actions>
        ...
        <v-btn
          color="success"
          variant="tonal"
          @click="bulkUpdateTradeTurnover"
          :loading="bulkUpdating"
          :disabled="bulkTurnoverValue < 0"
        >
          <v-icon icon="ri-global-line" class="mr-2" />
          Обновить все страны
        </v-btn>
  ...
  <!-- Модальное окно массового редактирования порогов -->
  <v-dialog
    v-model="bulkThresholdsDialog"
    max-width="800"
    persistent
  >
    <v-card>
      <v-card-title class="d-flex justify-space-between align-center">
        <span>Массовое редактирование порогов товарооборота</span>
        <v-btn
          icon="ri-close-line"
          variant="text"
          @click="cancelBulkThresholds"
        />
      </v-card-title>
      <v-divider />
      <v-card-text>
        <v-alert type="warning" variant="tonal" class="mb-4">
          <strong>Внимание:</strong> Это действие установит указанные пороги товарооборота для всех стран, кроме Вятки.
        </v-alert>
        ...
        <v-card class="mb-4" color="primary" variant="tonal">
          <v-card-text>
            <div class="d-flex align-center justify-space-between">
              <span class="text-subtitle-1 font-weight-bold">Режим ввода:</span>
              <v-btn-toggle
                v-model="bulkThresholdsInputMode"
                color="primary"
                mandatory
                divided
              >
            ...
            <v-alert class="mt-3" type="info" variant="tonal" density="compact">
              <template v-if="bulkThresholdsInputMode === 'absolute'">
                <strong>Абсолютная сумма:</strong> Введите точную сумму золота для каждого уровня
              </template>
              <template v-else>
                <strong>Процент:</strong> Укажите процент от предыдущего уровня (например, 200% от 1000 = 2000)
        ...
        <v-table>
          <thead>
            <tr>
              <th class="text-center">Уровень</th>
              <th class="text-center">Название</th>
              <th class="text-center" v-if="bulkThresholdsInputMode === 'absolute'">Порог (золото)</th>
              <th class="text-center" v-if="bulkThresholdsInputMode === 'percentage'">Процент от пред. уровня</th>
              <th class="text-center" v-if="bulkThresholdsInputMode === 'percentage'">Порог (золото)</th>
              <th class="text-center">Действия</th>
            </tr>
          </thead>
          <tbody>
            <tr
              v-for="(threshold, index) in bulkThresholds"
              :key="threshold.level"
            >
              <!-- Уровень -->
              <td class="text-center">
              ...
              <template v-if="bulkThresholdsInputMode === 'absolute'">
                <td class="text-center">
                  <v-text-field
                    ...
                    variant="outlined"
                    hide-details
                    @input="() => { recalculateBulkPercentage(index); recalculateBulkAllFromIndex(index); }"
                    @blur="recalculateBulkAllFromIndex(index)"
                  />
                </td>
              </template>

              <!-- Режим процентов -->
              <template v-if="bulkThresholdsInputMode === 'percentage'">
                <td class="text-center">
                  <v-text-field
                    v-if="index > 0"
                    v-model.number="threshold.percentage"
                    ...
                    variant="outlined"
                    hide-details
                    @input="() => { recalculateBulkFromPercentage(index); recalculateBulkAllFromIndex(index); }"
                    @blur="recalculateBulkAllFromIndex(index)"
                  />
                  <span v-else class="text-body-2 text-medium-emphasis">—</span>
                </td>
              ...
              <td class="text-center">
                <v-btn
                  ...
                  variant="text"
                  color="info"
                  @click="recalculateBulkAllFromIndex(index)"
                  title="Пересчитать"
                >
                  <v-icon icon="ri-calculator-line" />
                </v-btn>
                <v-btn
                  ...
                  variant="text"
                  color="error"
                  @click="removeBulkThreshold(index)"
                  title="Удалить"
                >
      ...
      <v-card-actions>
        ...
        <v-btn
          color="warning"
          variant="tonal"
          @click="saveBulkThresholds"
          :loading="savingBulkThresholds"
        >
          <v-icon icon="ri-save-line" class="mr-2" />
          Сохранить для всех стран
        </v-btn>
      </v-card-actions>
    </v-card>
  </v-dialog>
