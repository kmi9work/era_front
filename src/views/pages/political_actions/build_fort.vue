<script setup>
    ...
    await axios.get(`${import.meta.env.VITE_PROXY}/settlements.json`) 
      .then(response => {
        ...
        if (settlements.value.length > 0) {
          settlement_id.value = settlements.value[0].id;
        }
      })
  })
  ...
  async function runAction(noble_job_id, action_id){
    await axios.post(`${import.meta.env.VITE_PROXY}/political_actions.json`, {
        ...
        job_id: noble_job_id,
        params: {settlement_id: settlement_id.value}
      })
    emit('close-dialog')
  }
</script>
...
<template>
  <v-list>
    ...
    <v-list-item
      subtitle="Эффект"
    >Возводит Кремль в выбранном поселении.</v-list-item>
    ...
    <v-list-item>
      <v-select
        label="Выберите поселение"
        :items="settlements"
        v-model="settlement_id"
        item-title="name"
        item-value="id"
      ></v-select>
    </v-list-item>
